...
def log(R):
    ...
    v1 = (R[2][1]-R[1][2])/np.sin(theta)/2
    v2 = (R[0][2]-R[2][0])/np.sin(theta)/2
    v3 = (R[1][0]-R[0][1])/np.sin(theta)/2
...
def interpolateZYXEuler(euler1, euler2, t):
    #euler11 = np.array([euler1[2], euler1[1], euler1[0]])
    #euler21 = np.array([euler2[2], euler2[1], euler2[0]])
...
def main():
    ...
    if not glfw.init():
        return
    window = glfw.create_window(480,480,'2017029807-10-1', None,None)
    if not window:
        glfw.terminate()
        return
