...
newMq = np.array([[1, 0, -.1],
                  [0, 1, 0],
                  [0, 0, 1]])
newMe = np.array([[1, 0, .1],
                  [0, 1, 0],
                  [0, 0, 1]])

angleC = np.cos(10*np.pi/180)
angleS = np.sin(10*np.pi/180)
newMa = np.array([[angleC,-angleS,0],
                  [angleS,angleC,0],
                  [0,0,1]])
...
def main():
    if not glfw.init():
        return
    window = glfw.create_window(480,480,"2017029807-4-1", None,None)
    if not window:
        glfw.terminate()
        return
